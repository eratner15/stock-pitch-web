/**
 * Research Agent — plain Durable Object + Anthropic native SDK.
 * Bypasses AIChatAgent/AI SDK entirely to avoid Zod v4 schema issues.
 * Claude Sonnet as the brain, deterministic tools for data.
 */
...
TOOLS AVAILABLE: fetch_stock_price, fetch_sec_filing, store_portal
USE THEM SYSTEMATICALLY when researching a ticker.
...
WRITING RULES:
- Direct, specific, analytical institutional prose
...
const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description: 'Fetch the latest 10-K or 10-Q annual/quarterly filing from SEC EDGAR. Returns MD&A excerpt and risk factors.',
    input_schema: { type: 'object' as const, properties: { ticker: { type: 'string' }, filing_type: { type: 'string', enum: ['10-K', '10-Q'], description: '10-K for annual, 10-Q for quarterly' } }, required: ['ticker', 'filing_type'] },
  },
  {
    name: 'store_portal',
    description: 'Save your completed research analysis to the library. Call this when you have finished your research and have a price target.',
    input_schema: { type: 'object' as const, properties: { ticker: { type: 'string' }, company: { type: 'string' }, direction: { type: 'string', enum: ['long', 'short'] }, price_target: { type: 'number' }, current_price: { type: 'number' }, confidence: { type: 'number', description: '0-100 confidence score' }, summary: { type: 'string', description: '1-2 sentence bottom line up front' } }, required: ['ticker', 'company', 'direction', 'price_target', 'current_price', 'confidence', 'summary'] },
  },
];
...
export class ResearchAgent extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    // HTTP API: POST /research with {ticker}
    ...
    return new Response('Research Agent. POST /research with {"ticker":"META"}', { status: 200 });
  }
  ...
  async runResearch(ticker: string): Promise<Response> {
    ...
    const messages: Anthropic.MessageParam[] = [{
      role: 'user',
      content: `Research ${ticker} and produce a comprehensive institutional investment analysis. Use your tools to gather data, then write a detailed thesis with a price target. Be thorough.`,
    }];
        ...
        try {
          switch (block.name) {
            ...
            case 'fetch_sec_filing': {
              ...
              if (!cikResult) { result = { error: `CIK not found for ${args.ticker}` }; break; }
              if (String(args.filing_type) === '10-K') {
                const f = await fetchLatest10K(cikResult.cik);
                result = f ? { url: f.url, date: f.date, mda_excerpt: f.mda.slice(0, 15000), risk_factors: f.risks.slice(0, 8000) } : { error: 'No 10-K found' };
              } else {
                const f = await fetchLatest10Q(cikResult.cik);
                result = f ? { url: f.url, date: f.date, excerpt: f.text.slice(0, 5000) } : { error: 'No 10-Q found' };
              }
              break;
            }
            case 'store_portal': {
